...
class Algorithm extends Component
{
                ...
                foreach ($diag['instances'] as $instance_id => $instance) {
                    ...
                    if (empty($instance['conditions'])) {
                        ...
                        if ($instance_node['type'] === 'QuestionsSequence' && $instance['final_diagnosis_id'] === null) {
                            $this->algorithmService->manageQS($cached_data, $diag, $instance_node, $step, $consultation_nodes, $answers_hash_map, $qs_hash_map, $dependency_map, true);
                        }
                    ...
                    if (!empty($instance['conditions'])) {
                        foreach ($instance['conditions'] as $condition) {
                            ...
                            if ($instance_node['type'] === 'QuestionsSequence' && $instance['final_diagnosis_id'] === null) {
                                $this->algorithmService->manageQS($cached_data, $diag, $instance_node, $step, $consultation_nodes, $answers_hash_map, $qs_hash_map, $dependency_map, false, $answer_id);
                            }
                            ...
                            if ($node['type'] !== 'QuestionsSequence') {
                                ...
                                if ($instance['final_diagnosis_id'] === null) {
                                    $this->algorithmService->manageQS($cached_data, $diag, $node, $step, $consultation_nodes, $answers_hash_map, $qs_hash_map, $dependency_map, false, $answer_id);
                                }
                            }
                            ...
                            foreach ($instance['children'] as $child_node_id) {
                                ...
                                if ($child_node) {
                                    if ($child_node['type'] !== 'QuestionsSequence') {
                                        ...
                                        if ($instance['final_diagnosis_id'] === null) {
                                            $this->algorithmService->manageQS($cached_data, $diag, $child_node, $step, $consultation_nodes, $answers_hash_map, $qs_hash_map, $dependency_map, empty($instance['conditions']));
                                        }
                                    }
                                }
                            }
            ...
            foreach ($qs_per_dd as $dd_qs_id => $nodes_per_dd) {
                if (isset($nodes_per_dd[$value])) {
                    foreach ($nodes_per_dd[$value] as $qs_next_node_id => $qs_yes_answer_id) {
                        $this->displayNextNode($cached_data, $qs_next_node_id, $qs_yes_answer_id, null);
                    }
                }
            }
        }
                ...
                foreach ($nodes_per_cc as $next_node_dd_id => $next_nodes_id) {
                    foreach ($next_nodes_id as $node) {
                        ...
                        if (strpos(json_encode($qs_hash_map[$cc_id][$next_node_dd_id] ?? []), $node) > 0) {
                            dump($this->nodes_to_save[$node]);
                            dump("launched $node " . $this->nodes_to_save[$node]['answer_id']);
    ...
    public function updateLinkedNodesOfDob($value)
    {
        ...
        foreach ($birth_date_formulas as $node_id) {
            if ($full_nodes[$node_id]['label']['en'] === 'Age in days') {
                $this->saveNode("registration.$node_id", $node_id, $this->nodes_to_save[$node_id]['value'], $this->nodes_to_save[$node_id]['answer_id'], $this->nodes_to_save[$node_id]['answer_id'], $cached_data);
            }
        }
