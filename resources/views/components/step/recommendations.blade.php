      ...
      @foreach ($cc as $node_id => $answer_id)
        <div wire:key="{{ 'nodes-' . $node_id }}">
          @switch($full_nodes[$node_id]['display_format'])
            ...
            @case('String')
              <x-inputs.text step="consultation.{{ $cc_id }}" :node_id="$node_id" :cache_key="$cache_key" :is_background_calc="false" />
            @break
            ...
            @case('Formula')
              <x-inputs.text step="consultation.{{ $cc_id }}" :value="$nodes_to_save[$node_id]" :node_id="$node_id" :cache_key="$cache_key"
                :is_background_calc="true" />
            @break

            @case('Reference')
              <x-inputs.text step="consultation.{{ $cc_id }}" :node_id="$node_id" :value="$nodes_to_save[$node_id]" :cache_key="$cache_key"
                :is_background_calc="true" />
            @break
